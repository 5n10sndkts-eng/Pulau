...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { data: vendor, error: vendorError } = await supabase
      .from('vendors')
      .select('id, business_email, business_name, stripe_account_id, owner_id, onboarding_state')
      .eq('owner_id', user.id)
      .single()

    if (vendorError || !vendor) {
      return new Response(
        JSON.stringify({ success: false, error: 'Vendor not found for this user' }),
    ...
    if (!stripeAccountId) {
      ...
      if (updateError) {
        console.error('Failed to update vendor with Stripe account ID:', updateError)
        return new Response(
          JSON.stringify({ success: false, error: 'Failed to save Stripe account' }),
