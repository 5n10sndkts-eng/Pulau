...
describe('Process Refund Edge Function', () => {
  ...
  describe('[P0] Request Validation', () => {
    ...
    it('should accept valid refund request', () => {
      const body = { bookingId: 'booking-456', amount: 5000, reason: 'Customer request' }
  ...
  describe('[P0] Booking Validation', () => {
    it('should reject non-existent bookings', async () => {
      ...
      const result = await mockSupabaseClient.from('bookings').select().eq('id', 'invalid').single()
      expect(result.error).toBeDefined()
    })
  ...
  describe('[P0] Payment Validation', () => {
    it('should reject missing payment records', async () => {
      ...
      const result = await mockSupabaseClient.from('payments').select().eq('booking_id', 'invalid').single()
      expect(result.error).toBeDefined()
    })
    ...
    it('should reject fully refunded payments', () => {
      ...
      const maxRefundable = fullyRefundedPayment.amount - fullyRefundedPayment.refund_amount
  ...
  describe('[P0] Stripe Integration', () => {
    ...
    it('should handle Stripe errors gracefully', async () => {
      ...
      await expect(mockStripeRefundsCreate()).rejects.toThrow('Stripe API error')
    })
  ...
  describe('[P0] Database Updates', () => {
    it('should update payment status to refunded for full refunds', () => {
      ...
      const isFullRefund = refundAmount >= mockPayment.amount - mockPayment.refund_amount
