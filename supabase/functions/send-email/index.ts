...
type EmailData = ConfirmationEmailData | CancellationEmailData | ReminderEmailData
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorData = { statusCode: response.status, message: errorText, name: 'ResendError' }
      }
...
async function generateTicketPdf(bookingId: string): Promise<string | null> {
  try {
    ...
    const response = await fetch(`${supabaseUrl}/functions/v1/generate-ticket`, {
      method: 'POST',
      headers: {
 ...
 */
function validatePayload(payload: unknown): { valid: true; data: EmailPayload } | { valid: false; error: string } {
  ...
  if (!p.type || !['booking_confirmation', 'booking_cancellation', 'booking_reminder'].includes(p.type as string)) {
  ...
  const requiredFields = ['booking_reference', 'experience_name', 'experience_date', 'experience_time', 'guest_count', 'total_amount', 'currency', 'traveler_name']
...
function createNotificationMessage(
  type: 'booking_confirmed' | 'booking_cancelled' | 'reminder_24h' | 'reminder_2h',
...
async function createInAppNotification(
  ...
  const { title, message } = createNotificationMessage(params.type, params.experienceName)
  ...
  if (error) {
    ...
    console.log(`In-app notification created: type=${params.type}, user=${params.userId}`)
  }
}
...
function generateEmailContent(type: EmailType, data: EmailData): { subject: string; html: string } {
  switch (type) {
    case 'booking_confirmation': {
      const confirmData = data as ConfirmationEmailData
      const html = generateBookingConfirmationEmail(confirmData as BookingConfirmationData)
      return {
    ...
    case 'booking_cancellation': {
      ...
      const html = generateBookingCancellationEmail(cancelData as BookingCancellationData)
      return {
    ...
    case 'booking_reminder': {
      ...
      const html = generateBookingReminderEmail(reminderData as BookingReminderData)
      return {
  ...
  try {
    ...
    if (payload.type === 'booking_confirmation') {
      ...
      if (pdfBase64) {
        ...
        console.log(`No PDF ticket attached for booking ${payload.booking_id} (generation failed or skipped)`)
      }
    }
    ...
    console.log(`Email sent: type=${payload.type}, booking=${payload.booking_id}, message_id=${result.id}`)
    ...
    try {
      const rawPayload = await req.clone().json().catch(() => ({})) as Record<string, unknown>
      if (rawPayload.booking_id) {
        ...
        await createAuditLog(supabase, {
          ...
          metadata: {
            ...
            recipient_hash: rawPayload.to ? hashEmail(rawPayload.to as string) : 'unknown',
            status: 'failed',
            error: errorMessage,
          },
        })
