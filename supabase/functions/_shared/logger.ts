...
function redactPII(obj: unknown): unknown {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (PII_KEYS.has(lowerKey) || lowerKey.includes('password') || lowerKey.includes('secret')) {
      result[key] = '[REDACTED]'
...
export class Logger {
  ...
  requestEnd(status: number, data?: Record<string, unknown>): void {
    ...
    const log = this.createLog(level, `Request completed with status ${status}`, {
      status,
      durationMs: duration,
      ...data,
    })
   ...
   */
  dbOperation(operation: string, table: string, data?: Record<string, unknown>): void {
   ...
   */
  externalCall(service: string, operation: string, data?: Record<string, unknown>): void {
