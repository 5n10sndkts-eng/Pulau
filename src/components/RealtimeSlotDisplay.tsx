...
export function RealtimeSlotDisplay({
  ...
  if (slots.length === 0) {
    return (
      <div className="text-center py-12 text-muted-foreground">
        ...
        <p className="text-sm font-medium">No time slots available for this date</p>
  ...
  return (
    <div className="space-y-4">
      ...
      <AnimatePresence>
        {(connectionState === 'disconnected' || connectionState === 'error' || isStale) && (
          <motion.div
      ...
      <div className="space-y-3">
        {slots.map(slot => {
          ...
          const isLowAvailability = slot.available_count > 0 && slot.available_count <= 3
          ...
          return (
            ...
            >
              <button
                onClick={() => isAvailable && onSlotSelect && onSlotSelect(slot)}
      ...
      {connectionState === 'connected' && lastUpdate && (
        <p className="text-xs text-center text-muted-foreground">
          Live updates • Last refreshed {new Date(lastUpdate).toLocaleTimeString()}
        </p>
      )}
    </div>
  )
