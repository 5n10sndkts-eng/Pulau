    ...
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Make sure everything looks good before we continue</p>
        </div>
      </div>
      ...
      {hasConflicts ? (
        ...
        <Alert className="border-success bg-success/5">
          ...
          <AlertDescription className="text-success">No scheduling conflicts found - you're all set!</AlertDescription>
        </Alert>
      )}
          ...
          <Card key={date} className="p-5">
            ...
            <div className="space-y-3">
              {itemsByDate[date].map((item, itemIndex) => {
                ...
                return (
                  <div key={itemIndex} className="flex gap-3 p-3 bg-muted/50 rounded-lg">
                    <img
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-semibold text-sm leading-tight mb-1">{experience.title}</h3>
                      <p className="text-xs text-muted-foreground mb-1">
                        {item.time} • {experience.duration} • {item.guests} guests
                      </p>
