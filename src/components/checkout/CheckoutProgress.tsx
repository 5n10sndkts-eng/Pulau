          ...
          return (
            <div key={step.id} className="flex items-center flex-1">
              <button
                ...
                className={`flex flex-col items-center gap-2 relative transition-all ${isCompleted || isCurrent ? 'opacity-100 cursor-pointer' : 'opacity-50 cursor-not-allowed'
                  }`}
              ...
              >
                ...
                <span
                  className={`text-xs font-medium whitespace-nowrap ${isCurrent ? 'text-foreground' : 'text-muted-foreground'
                    }`}
                >
                  {step.label}
                </span>
              ...
              {index < steps.length - 1 && (
                <div className="flex-1 h-0.5 mx-2 relative" style={{ top: '-20px' }}>
                  ...
                  <div
                    className={`absolute inset-0 bg-primary transition-all duration-300 ${index < currentIndex ? 'w-full' : 'w-0'
                      }`}
                  />
                </div>
              )}
