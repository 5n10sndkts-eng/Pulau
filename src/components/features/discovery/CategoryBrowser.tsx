...
import { ArrowLeft, Search, Clock, Users, Star, Heart, Plus, Check, Zap, Wallet } from 'lucide-react'
...
export function CategoryBrowser({
  ...
  const remainingBudget = budgetPref ? getRemainingBudget(budgetPref, trip.total) : null

  // Filter experiences - handle "underbudget" specially
  const filteredExps = activeFilter === 'underbudget' && remainingBudget !== null
  ...
  const handleQuickAddWithAnimation = (exp: Experience, buttonRect: DOMRect) => {
    ...
    if (isInTrip) {
      ...
      triggerFly(buttonRect.x + buttonRect.width / 2, buttonRect.y + buttonRect.height / 2)
      onQuickAdd(exp)
    }
  }
            ...
            {filters.map((filter) => (
              <Badge
                ...
                className={`cursor-pointer whitespace-nowrap flex items-center gap-1 ${
                  filter.id === 'underbudget' && activeFilter !== 'underbudget'
              ...
              >
                ...
                {filter.id === 'underbudget' && <Wallet className="w-3 h-3" />}
          ...
          <div className="space-y-6">
            {filteredExps.map((exp) => (
              <ExperienceCard
                ...
                onQuickAdd={(buttonRect) => handleQuickAddWithAnimation(exp, buttonRect)}
