...
import { getExperienceById, formatPrice, formatDateRange, getDayLabel, getDaysBetween, checkForConflicts, findNextAvailableSlot } from '@/lib/helpers'
import { ArrowLeft, Share2, Clock, Trash2, Calendar, Edit2, Check, AlertTriangle, Wand2 } from 'lucide-react'
...
export function TripBuilder({ trip, onBack, onRemoveItem, onCheckout, onUpdateTrip, readOnly = false }: TripBuilderProps) {
  const [viewMode, setViewMode] = useState<'list' | 'calendar'>('list')
  const [isEditingTitle, setIsEditingTitle] = useState(false)
  const [editedTitle, setEditedTitle] = useState(trip.destination || 'Your Trip') // Fallback title
          ...
          <div className="flex-1">
            {isEditingTitle ? (
              <div className="flex items-center gap-2">
                ...
                />
                <Button size="icon" variant="ghost" className="h-8 w-8" onClick={handleSaveTitle} aria-label="Save title">
                  <Check className="w-4 h-4 text-green-500" />
                </Button>
              </div>
            ) : (
              <div className="flex items-center gap-2 cursor-pointer group" onClick={() => !readOnly && setIsEditingTitle(true)}>
                <h1 className="font-display text-2xl font-bold hover:underline decoration-dashed decoration-1 underline-offset-4">
                  {trip.name || trip.destination === 'dest_bali' ? 'Your Bali Trip' : 'Your Trip'}
                </h1>
                {!readOnly && <Edit2 className="w-4 h-4 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />}
              </div>
            )}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              {!readOnly && trip.startDate && (
                <Button variant="outline" size="sm" onClick={handleAutoSchedule} className="gap-2 text-primary hover:text-primary">
