...
import {
  validateExperienceForPublishing,
  ValidationResult,
...
export function VendorExperiences({
  session,
  onBack,
  onNavigateToCreate,
  onNavigateToEdit,
  onNavigateToAvailability,
}: VendorExperiencesProps) {
  const [experiences, setExperiences] = useState<Experience[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [vendorCapabilities, setVendorCapabilities] =
    useState<VendorCapabilities | null>(null);
  const [updatingInstantBook, setUpdatingInstantBook] = useState<string | null>(
    null,
  );
  ...
  const handleInstantBookToggle = async (
    ...
    try {
      ...
      setExperiences((prev) =>
        prev.map((exp) =>
          exp.id === experienceId
            ? { ...exp, instantBookEnabled: enabled }
            : exp,
        ),
      );
      toast.success(
        enabled ? 'Instant Book enabled!' : 'Switched to Request to Book',
      );
    } catch (error) {
  ...
  const handleCutoffChange = async (experienceId: string, hours: string) => {
    ...
    try {
      await vendorService.updateExperienceCutoffHours(
        experienceId,
        cutoffHours,
      );
      setExperiences((prev) =>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-white/80 mt-1">
                {experiences.length}{' '}
                {experiences.length === 1 ? 'experience' : 'experiences'} listed
              </p>
            </div>
            <Button
        ...
        {isLoading ? (
          ...
          <Card className="p-12 text-center">
            <div className="max-w-md mx-auto space-y-4">
              ...
              <Button size="lg" onClick={onNavigateToCreate}>
                <Plus className="h-5 w-5 mr-2" />
                  ...
                  <div className="flex-1 p-6">
                    ...
                    <div className="flex items-center justify-between rounded-lg border p-3 mb-4 bg-muted/30">
                      <div className="flex items-center gap-2">
                        <Zap
                          className={`h-4 w-4 ${experience.instantBookEnabled ? 'text-yellow-500' : 'text-muted-foreground'}`}
                        />
                        <div>
                      ...
                      <TooltipProvider>
                        <Tooltip>
                            ...
                            <div>
                              <Switch
                                ...
                                disabled={
                                  ...
                                  updatingInstantBook === experience.id
                                }
                          ...
                          {!vendorCapabilities?.canEnableInstantBook && (
                            <TooltipContent>
                              <p>
                                Complete payment setup to enable Instant Book
                              </p>
                            </TooltipContent>
                          )}
                        </Tooltip>
                    ...
                    <div className="flex items-center justify-between rounded-lg border p-3 mb-4 bg-muted/30">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          handleCutoffChange(experience.id, value)
                        }
                    ...
                    <div className="flex md:flex-col gap-2">
                      ...
                      <div className="flex gap-2 w-full">
                        ...
                        <Button
                          ...
                          className="flex-1"
                          onClick={() =>
                            onNavigateToAvailability(experience.id)
                          }
                        >
