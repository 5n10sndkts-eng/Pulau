      ...
      <DialogContent className="sm:max-w-md">
          ...
          {validationResult.errors.map((error, index) => (
            <div key={index} className="flex items-center justify-between p-2 rounded-md bg-muted/50">
              <div className="flex items-center gap-3">
                ...
                <span className={`text-sm ${error.pass ? 'text-foreground' : 'text-destructive font-medium'}`}>
                  {error.message}
                </span>
              </div>
              {error.pass && (
        ...
        <DialogFooter className="sm:justify-between flex-row-reverse gap-2">
          {validationResult.valid ? (
            ...
            <Button onClick={onClose} variant="secondary" className="w-full sm:w-auto ml-auto">
              I'll fix these
            </Button>
          )}
        </DialogFooter>
