...
export function ValidationCard({
  ...
  return (
    ...
    >
      <Card
        className={`overflow-hidden border-2 ${valid ? 'border-teal-500' : 'border-red-500'}`}
      >
        {/* Status Header */}
        <div
          className={`p-4 text-white flex items-center gap-3 ${valid ? 'bg-teal-600' : 'bg-red-600'}`}
        >
          {valid ? (
        ...
        <div className="p-6 space-y-6 bg-white">
          {/* Main Message */}
          <div
            className={`p-3 rounded-lg text-sm font-medium ${valid ? 'bg-teal-50 text-teal-800' : 'bg-red-50 text-red-800'}`}
          >
            {message}
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                ...
                {booking.items.map((item) => (
                  <div
                    key={item.id}
                    className={`p-4 rounded-xl border ${item.is_today ? 'bg-teal-50/50 border-teal-100' : 'bg-muted/30 border-muted'}`}
                  >
                    <div className="flex items-start gap-3">
                      <div
                        className={`p-2 rounded-lg ${item.is_today ? 'bg-teal-100 text-teal-700' : 'bg-muted text-muted-foreground'}`}
                      >
