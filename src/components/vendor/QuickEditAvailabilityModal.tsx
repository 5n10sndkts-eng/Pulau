...
export function QuickEditAvailabilityModal({
  ...
  const checkBookingConflicts = () => {
    ...
    const conflicts = safeBookings.filter(booking => {
      ...
      return booking.trip.items.some(item => 
  ...
  const handleSave = () => {
    ...
    if (!isBlocked && slots < totalBookedSlots) {
      toast.error(`Cannot reduce slots below ${totalBookedSlots} (current bookings)`)
      return
    }
  ...
  const performSave = (forceBlock = false) => {
    ...
    try {
      ...
      const updatedData = safeAvailability.filter(a => a.date !== selectedDate)
      updatedData.push(newAvailability)
      
      setAvailabilityData(updatedData)
      
      toast.success(`Availability updated for ${new Date(selectedDate).toLocaleDateString()}`)
        ...
        {showConflictWarning ? (
          <div className="py-6 space-y-4">
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {conflictingBookings.length} existing booking{conflictingBookings.length !== 1 ? 's' : ''} on this date
              </AlertDescription>
            </Alert>
