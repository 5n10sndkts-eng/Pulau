...
export function NavigationShellWithRouter({
    ...
    const activeTab = tabs.find(tab => {
        if (tab.id === '/') return location.pathname === '/' || location.pathname.startsWith('/trip')
    ...
    return (
        <div className="relative min-h-screen flex flex-col bg-background font-body">
            ...
            <header className="sticky top-0 z-40 w-full glass border-b border-primary/10 px-6 py-4 flex items-center justify-between">
                ...
                >
                    <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center">
                        <Map className="w-5 h-5 text-primary-foreground" aria-hidden="true" />
                    </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    >
                        <UserCircle className="w-6 h-6 text-muted-foreground" aria-hidden="true" />
                    </Button>
                </div>
            </header>
            ...
            {/* Bottom Navigation */}
            {showNav && (
                <nav className="fixed bottom-0 left-0 right-0 z-50 px-4 pb-4" aria-label="Main Navigation">
                        ...
                        {tabs.map((tab) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <Icon className={cn("w-6 h-6 transition-transform group-hover:scale-110", isActive && "fill-current")} aria-hidden="true" />
