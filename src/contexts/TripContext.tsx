...
export function TripProvider({ children }: { children: React.ReactNode }) {
  ...
  const saveTripToBackend = useCallback(async (newTrip: Trip) => {
    if (user) {
      try {
        const saved = await tripService.saveTrip({ ...newTrip, userId: user.id })
  ...
  // Public Actions
  const replaceTrip = useCallback((newTrip: Trip) => {
      setTrip(newTrip)
      // Debounce saving could be added here, but direct save for now
      saveTripToBackend(newTrip)
  }, [saveTripToBackend])

  const addToTrip = useCallback((experienceId: string, guests: number, totalPrice: number) => {
    setTrip(prev => {
      const existingItemIndex = prev.items.findIndex(item => item.experienceId === experienceId)
  ...
  const removeFromTrip = useCallback((experienceId: string) => {
    setTrip(prev => {
       const newItems = prev.items.filter(item => item.experienceId !== experienceId)
       const recalculated = calculateTripTotal(newItems)
       const newTrip = { ...prev, items: newItems, ...recalculated }
       saveTripToBackend(newTrip)
       return newTrip
