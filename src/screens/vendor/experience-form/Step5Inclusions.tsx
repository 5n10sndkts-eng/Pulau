...
export function Step5Inclusions({ data, onNext, onBack }: WizardStepProps & { inclusions: ExperienceInclusionRecord[], setInclusions: (i: ExperienceInclusionRecord[]) => void }) {
    ...
    const props = arguments[0] as WizardStepProps & { inclusions: ExperienceInclusionRecord[], setInclusions: (i: ExperienceInclusionRecord[]) => void }
    const { inclusions, setInclusions } = props

    const addInclusion = (isIncluded: boolean, text: string, setInput: (s: string) => void) => {
        if (!text.trim()) return
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
            <div className="space-y-2">
                <h2 className="text-2xl font-display font-bold">Inclusions & Exclusions</h2>
                <p className="text-muted-foreground">Be transparent about what is covered.</p>
            </div>
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                        <Input
                            ...
                            onKeyDown={(e) => e.key === 'Enter' && addInclusion(true, incInput, setIncInput)}
                        />
                        <Button size="icon" onClick={() => addInclusion(true, incInput, setIncInput)}>
                            <Plus className="w-4 h-4" />
                        </Button>
                    </div>
                    <ul className="space-y-2">
                        {inclusions.filter(i => i.isIncluded).map(item => (
                            <li key={item.id} className="flex items-center justify-between bg-emerald-50/50 p-2 rounded-md border border-emerald-100">
                                <span className="text-sm">{item.itemText}</span>
                                <button onClick={() => removeInclusion(item.id)} className="text-muted-foreground hover:text-red-500">
                                    <X className="w-4 h-4" />
                                </button>
                            </li>
                        ))}
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex gap-2">
                        <Input
                            ...
                            onKeyDown={(e) => e.key === 'Enter' && addInclusion(false, excInput, setExcInput)}
                        />
                        <Button size="icon" variant="secondary" onClick={() => addInclusion(false, excInput, setExcInput)}>
                            <Plus className="w-4 h-4" />
                        </Button>
                    </div>
                    <ul className="space-y-2">
                        {inclusions.filter(i => !i.isIncluded).map(item => (
                            <li key={item.id} className="flex items-center justify-between bg-red-50/50 p-2 rounded-md border border-red-100">
                                <span className="text-sm">{item.itemText}</span>
                                <button onClick={() => removeInclusion(item.id)} className="text-muted-foreground hover:text-red-500">
                                    <X className="w-4 h-4" />
                                </button>
