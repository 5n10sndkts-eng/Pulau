...
export function TripsDashboard({
  ...
  const [selectedTab, setSelectedTab] = useState<'upcoming' | 'past' | 'all'>(
    'upcoming',
  );
  ...
  const pastBookings = safeBookings
    .filter((b) => {
      if (!b.trip.endDate) {
  ...
  const renderBookingCard = (booking: Booking) => {
    ...
    const startDate = booking.trip.startDate
      ...
      : null;
    const endDate = booking.trip.endDate
  ...
  const renderEmptyState = (type: 'upcoming' | 'past' | 'all') => {
    ...
    return (
      <div className="text-center py-16 space-y-4">
        ...
        <h2 className="font-display text-xl font-semibold">{getEmptyText()}</h2>
    ...
    <div className="min-h-screen bg-background pb-24">
      <div className="max-w-4xl mx-auto p-6">
        ...
        <Tabs
          value={selectedTab}
          onValueChange={(v) =>
            setSelectedTab(v as 'upcoming' | 'past' | 'all')
          }
        >
