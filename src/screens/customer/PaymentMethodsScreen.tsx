...
export function PaymentMethodsScreen({ onBack }: PaymentMethodsScreenProps) {
  ...
  const [paymentMethods, setPaymentMethods] = useKV<PaymentMethod[]>(
    'paymentMethods',
    [],
  );
  ...
  const handleDelete = (id: string) => {
    setPaymentMethods((current) => {
      const base = current || [];
      return base.map((pm) =>
        pm.id === id ? { ...pm, deletedAt: new Date().toISOString() } : pm,
      );
    });
      ...
      <Dialog open={showAddCard} onOpenChange={setShowAddCard}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Your card information is securely tokenized. We never store your
              full card number.
            </DialogDescription>
          </DialogHeader>
