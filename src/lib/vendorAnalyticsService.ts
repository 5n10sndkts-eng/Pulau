...
function getPreviousPeriodRange(period: TimePeriod): {
  startDate: Date;
  endDate: Date;
} {
...
export async function getVendorRevenueStats(
  ...
  try {
    ...
    const { data: allPayments, error: paymentsError } =
      await buildRevenueQuery();
    ...
    // Calculate totals
    const totalRevenue = vendorPayments.reduce(
    ...
    const currentPeriodRevenue = currentPeriodPayments.reduce(
      (sum, p) => sum + (p.amount || 0),
      0,
    );
...
function generateMockExperienceMetrics(): ExperiencePerformanceMetrics[] {
  ...
  return MOCK_EXPERIENCE_TITLES.slice(0, count).map((title, index) => {
    ...
    return {
      ...
      bookedSlots,
      averageRating:
        reviewCount > 0 ? Math.round(Math.random() * 15 + 35) / 10 : null, // 3.5-5.0
      reviewCount,
...
export async function getExperiencePerformanceMetrics(
  ...
  if (USE_MOCK_DATA) {
    ...
    const avgUtilization =
      sorted.length > 0
        ? Math.round(
            sorted.reduce((sum, e) => sum + e.slotUtilization, 0) /
              sorted.length,
          )
        : 0;
  ...
  try {
    ...
    for (const slot of slotData || []) {
      const existing = slotStats.get(slot.experience_id) || {
        total: 0,
        booked: 0,
      };
    ...
    for (const item of tripItems || []) {
      ...
      for (const booking of bookings) {
        ...
        const existing = bookingStats.get(item.experience_id) || {
          bookings: 0,
          revenue: 0,
        };
    ...
    for (const review of reviews || []) {
      const existing = reviewStats.get(review.experience_id) || {
        sum: 0,
        count: 0,
      };
    ...
    const metrics: ExperiencePerformanceMetrics[] = experiences.map((exp) => {
      ...
      const utilization =
        slots.total > 0 ? Math.round((slots.booked / slots.total) * 100) : 0;
      const avgRating =
        review.count > 0
  ...
  try {
    ...
    for (const payout of data?.pending || []) {
      payouts.push({
        ...
        createdAt: new Date(
          payout.arrival_date * 1000 - 2 * 24 * 60 * 60 * 1000,
        ),
...
function generateMockFunnelData(period: TimePeriod): FunnelData {
  ...
  const viewsToAddRate = 0.15 + Math.random() * 0.1;
  const addToCheckoutRate = 0.4 + Math.random() * 0.2;
  const checkoutToConfirmRate = 0.7 + Math.random() * 0.15;
  ...
  const stages: FunnelStage[] = [
    ...
    },
    {
      ...
      conversionFromPrevious:
        addToTrip > 0 ? (checkoutStarted / addToTrip) * 100 : 0,
    },
    {
      name: 'Confirmed',
      value: confirmed,
      ...
      conversionFromPrevious:
        checkoutStarted > 0 ? (confirmed / checkoutStarted) * 100 : 0,
    },
  ];
