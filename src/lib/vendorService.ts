...
export const vendorService = {
  ...
  getVendorByUserId: async (userId: string): Promise<Vendor | null> => {
    ...
    return {
      ...
      status:
        (data.status as 'pending_verification' | 'active' | 'suspended') ||
  ...
  getVendorExperiences: async (vendorId: string): Promise<Experience[]> => {
    if (USE_MOCK_DATA) {
      ...
      return experiences.filter(
        (e) => e.provider.id === vendorId || e.vendorId === vendorId,
      );
    }
  ...
  updateExperienceInstantBook: async (
    experienceId: string,
    ...
    console.warn(
      'instant_book_enabled column not yet migrated to experiences table',
    );
    return;
   ...
   */
  updateExperienceCutoffHours: async (
    experienceId: string,
    cutoffHours: number,
  ): Promise<void> => {
    if (USE_MOCK_DATA) {
    ...
    if (error) {
      if (import.meta.env.DEV)
        console.error('Error updating cutoff hours:', error);
      throw error;
    }
  },
};
