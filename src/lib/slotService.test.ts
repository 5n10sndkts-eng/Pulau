...
describe('slotService', () => {
    ...
    describe('createSlot', () => {
        it('successfully creates a slot', async () => {
            ...
            mockChain.then = (resolve: any) => resolve({ data: mockSlot, error: null })
    ...
    describe('createBulkSlots', () => {
        it('successfully creates multiple slots', async () => {
            ...
            const result = await createBulkSlots([
                { experienceId: mockExperienceId, slotDate: '2026-06-01', slotTime: '10:00', totalCapacity: 10 }
            ])
    ...
    describe('updateSlot', () => {
        it('successfully updates a slot and logs audit', async () => {
            const mockChain = createMockSupabase()
            mockChain.then = (resolve: any) => resolve({ data: { ...mockSlot, total_capacity: 15 }, error: null })
            vi.mocked(supabase.from).mockReturnValue(mockChain as any)
    ...
    describe('blockSlot', () => {
        it('blocks a slot and creates audit log', async () => {
            const mockChain = createMockSupabase()
            mockChain.then = (resolve: any) => resolve({ data: { ...mockSlot, is_blocked: true }, error: null })
            vi.mocked(supabase.from).mockReturnValue(mockChain as any)
    ...
    describe('incrementAvailability', () => {
        it('increments availability', async () => {
            const mockChain = createMockSupabase()
            mockChain.then = (resolve: any) => resolve({ data: { ...mockSlot, available_count: 11 }, error: null })
            vi.mocked(supabase.from).mockReturnValue(mockChain as any)
