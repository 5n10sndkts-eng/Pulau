...
export const BUDGET_CAPS: Record<NonNullable<UserPreferences['budget']>, number> = {
  budget: 300,
  midrange: 800,
  luxury: 2000,
}
 ...
 */
export function getRemainingBudget(budget: UserPreferences['budget'], currentTotal: number): number {
 ...
 */
export function getBudgetStatus(budget: UserPreferences['budget'], currentTotal: number): BudgetStatus {
  const cap = getBudgetCap(budget)
 ...
 */
export function formatBudgetMessage(budget: UserPreferences['budget'], currentTotal: number): string {
