...
export function parseDuration(duration: string): number {
  const match = duration.match(
    /(\d+(?:\.\d+)?)\s*(hours?|hrs?|h|minutes?|mins?|m)/i,
  );
...
export function detectConflicts(
  ...
  for (let i = 0; i < dayItems.length; i++) {
    for (let j = i + 1; j < dayItems.length; j++) {
      ...
      const duration1 = exp1.durationHours
        ? exp1.durationHours * 60
        : parseDuration(exp1.duration);
      const duration2 = exp2.durationHours
        ? exp2.durationHours * 60
        : parseDuration(exp2.duration);
