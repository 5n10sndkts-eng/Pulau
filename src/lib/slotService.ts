...
export type ExperienceSlotInsert =
  Database['public']['Tables']['experience_slots']['Insert'];
export type ExperienceSlotUpdate =
  Database['public']['Tables']['experience_slots']['Update'];
...
export async function createBulkSlots(
  ...
  for (const input of inputs) {
    ...
    if (res.data) {
      ...
      result.failed++;
      result.errors.push(
        res.error || `Failed: ${input.slotDate} ${input.slotTime}`,
      );
    }
  }
...
export async function decrementAvailabilityWithLock(
  ...
  try {
    ...
    if (fetchError || !updatedSlot) {
      ...
      console.warn(
        'Slot decremented but could not fetch updated data:',
        fetchError,
      );
    }
    ...
    return {
      ...
      error:
        err instanceof Error
          ? err.message
          : 'Unexpected error during inventory decrement',
    };
  }
}
...
export function isSlotWithinCutoff(
  ...
  const cutoffTime = new Date(
    slotDateTime.getTime() - cutoffHours * 60 * 60 * 1000,
  );
 ...
 */
export function filterSlotsByCutoff<
  T extends { slot_date: string; slot_time: string },
>(slots: T[], cutoffHours: number): T[] {
  return slots.filter(
    (slot) => !isSlotWithinCutoff(slot.slot_date, slot.slot_time, cutoffHours),
  );
}
