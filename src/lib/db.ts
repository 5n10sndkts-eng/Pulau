...
export function toExperienceRecord(exp: Experience): ExperienceRecord {
    return {
        ...
        destinationId: typeof exp.destination === 'string' ? exp.destination : (exp.destination as any).id,
...
export function getActiveExperiences(): Experience[] {
    ...
    return mockExperiences.filter(exp => !exp.status || exp.status === ExperienceStatus.Active)
}
