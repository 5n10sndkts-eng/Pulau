...
describe('useRealtimeSlots', () => {
  ...
  it('should call callback when slot changes', async () => {
    ...
    const { result } = renderHook(() => useRealtimeSlots('exp-789', userCallback))
  ...
  it('should not retry when retryOnError is false', async () => {
    ...
    expect(realtimeService.subscribeToSlotAvailability).toHaveBeenCalledTimes(1)

    consoleSpy.mockRestore()
  })

  it('should resubscribe when experienceId changes', async () => {
    const mockUnsubscribe = vi.mocked(realtimeService.unsubscribe).mockResolvedValue()
