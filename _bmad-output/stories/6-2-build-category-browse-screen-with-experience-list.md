...
1. **Given** I tapped a category from Story 6.1 **When** the category browse screen loads **Then** header shows: back arrow (using React Router), category title (e.g., "Water Adventures"), search icon (Lucide React)
...
- [ ] Task 1: Create category browse page layout (AC: #1, #4)
  - [ ] Create `src/pages/CategoryBrowse.tsx`
  - [ ] Build header with back arrow (React Router Link), dynamic category title, search icon (Lucide React Search)
...
- [ ] Task 2: Create ExperienceCard component (AC: #3)
  ...
  - [ ] Create quick stats row with Lucide React icons: Clock (duration), Users (group size), Star (rating)
...
- [ ] Task 7: Implement navigation (AC: #3)
  ...
  - [ ] Handle back navigation to return to browse screen preserving scroll position
...
**Issues Fixed:**
1. ✅ File paths: `src/screens/browse/CategoryBrowseScreen.tsx` → `src/pages/CategoryBrowse.tsx`
2. ✅ Data layer: Supabase table queries → KV store with key pattern `experiences:category:{categoryId}`
...
10. ✅ Joins: SQL joins → Nested objects in KV store data
11. ✅ Aggregations: SQL avg() → Client-side JavaScript calculations
