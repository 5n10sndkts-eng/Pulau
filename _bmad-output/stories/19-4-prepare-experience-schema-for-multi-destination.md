...
1. **Given** a vendor creates an experience **When** the creation form loads **Then** destination is pre-selected based on vendor's location or selectable **And** experience record includes destination_id
2. **And** experience only appears in browse/search when matching user's selected destination
...
- [ ] Task 4: Update experience queries to filter by destination (AC: #2)
  ...
  - [ ] Ensure browse/search only shows matching destination experiences
