...
describe('Slot Booking Concurrency Tests', () => {
  ...
  describe('Race Condition Prevention', () => {
    ...
    it('CRITICAL: should reject third concurrent request when only 2 spots', async () => {
      ...
      expect(failedResult.reason.message).toMatch(/not available|insufficient inventory/i)
    })
...
describe('Analytics Performance Benchmarks', () => {
  ...
  describe('Response Time Requirements', () => {
    ...
    it('should load dashboard in under 1000ms (medium vendor)', async () => {
      const mediumVendor = await createVendorWithBookings({ bookingCount: 500 })
    ...
    it('should load dashboard in under 2000ms (large vendor)', async () => {
      const largeVendor = await createVendorWithBookings({ bookingCount: 5000 })
  ...
  describe('Query Efficiency', () => {
    ...
    it('should scan minimal rows (< 500 for 30-day period)', async () => {
      const queryPlan = await supabase.rpc('explain_query', {
        ...
        params: { p_vendor_id: testVendorId, p_start_date: '...', p_end_date: '...' }
      })
...
test.describe('Authentication Security E2E', () => {
  ...
  test('should successfully register with strong password', async ({ page }) => {
    await page.goto('/auth/register')
  ...
  test('should not allow mock auth bypass in production build', async ({ page }) => {
...
test.describe('Concurrent Booking Prevention E2E', () => {
  test('should handle last spot booking correctly with 2 users', async () => {
    ...
    const failedPage = successes.length > 0 && results[0].status === 'fulfilled' ? page2 : page1
    await expect(failedPage.locator('text=/no longer available/i')).toBeVisible()
  ...
  test('should update availability in real-time for other users', async () => {
    ...
    // Both see 5 spots
    await expect(page1.locator('[data-testid="available-count"]')).toHaveText('5')
    await expect(page2.locator('[data-testid="available-count"]')).toHaveText('5')
    ...
    await expect(page2.locator('[data-testid="available-count"]')).toHaveText('3', {
      timeout: 3000
    })
