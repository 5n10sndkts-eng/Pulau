...
- **Visual Trust**: Video-first verified reviews that show the _actual_ experience, not just polished marketing photos.
...
- **Result**: Ticket QR code downloads _instantly_ to local storage.
...
- **Offline First Engine**: We will implement **WatermelonDB** (or RxDB) to maintain a local replica of the `bookings` and `tickets` tables. This ensures the "Ticket in Pocket" works in airplane mode.
  - _Constraint_: Must handle conflict resolution if a Vendor edits a booking offline while a User cancels online (Last-Write-Wins favored for Vendor).
...
- **Push Notification Strategy**:
  ...
  - _Trigger_: `bookings` table INSERT -> Edge Function -> OneSignal API -> User Device.
...
- _Partial_ Journey 1 (Sam): User books instantly (mobile web) and relies on PWA Cache + Email PDF for offline ticket.
