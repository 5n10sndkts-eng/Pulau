...
test.describe('PWA Offline Functionality [Epic 26]', () => {
  ...
  test('AC-26.6-1: should load ticket details from cache when offline', async ({
    page,
    context,
  }) => {
  ...
  test('AC-26.6-2: should display offline indicator and disable actions', async ({
    page,
    context,
  }) => {
  ...
  test('AC-26.6-3: should sync and remove indicator when network restores', async ({
    page,
    context,
  }) => {
    ...
    await expect(page.getByText(/offline mode/i)).toBeHidden({
      timeout: 10000,
    });
  });
});
