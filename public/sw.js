    ...
    if (cachedResponse) {
      ...
      if (cacheTimestamp) {
        ...
        if (age > CACHE_EXPIRATION_MS) {
          console.warn('[Service Worker] Cached data expired (>30 days):', request.url)
          // Return expired data but log warning
          // In production, might want to delete expired entries
        } else {
          console.log('[Service Worker] Serving from cache (age:', Math.floor(age / 1000 / 60 / 60), 'hours)')
        }
      }
...
function isAsset(url) {
  const assetExtensions = ['.js', '.css', '.png', '.jpg', '.jpeg', '.svg', '.woff', '.woff2', '.ttf']
