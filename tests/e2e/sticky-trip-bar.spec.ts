...
test.describe('Epic 33: Sticky Trip Bar', () => {
  ...
  test('should show sticky trip bar when items are added to trip @p0 @sticky-bar', async ({ page }) => {
    ...
    console.log('Body HTML snippet:', await page.innerHTML('body').then(h => h.substring(0, 500)));
    
    // Wait for the search input to be available
    const searchInput = page.getByPlaceholder(/search experiences, categories/i);
    ...
    await expect(page.locator('button').filter({ hasText: /checkout/i })).toBeVisible();
  });

  test('should be accessible via keyboard @p0 @a11y @sticky-bar', async ({ page, auth }) => {
    ...
    await page.locator('button').filter({ hasText: /checkout/i }).click();
  ...
  test('should show correct trip count @p1 @sticky-bar', async ({ page }) => {
    ...
    await page.getByPlaceholder(/search experiences, categories/i).fill('beach');
    await page.waitForTimeout(1000);
    
    await page.getByRole('button', { name: /add/i }).first().click();
    // Scope search to the sticky bar to avoid matching prices on experience cards
    await expect(page.getByLabel(/View trip summary/i).getByText(/\$\d+/)).toBeVisible();
    
    // Verify checkout button is visible
    await expect(page.locator('button').filter({ hasText: /checkout/i })).toBeVisible();
  });

  test('should persist trip state across navigation @p1 @sticky-bar', async ({ page }) => {
    await page.goto('/explore');
    await page.waitForLoadState('networkidle');
    await page.getByPlaceholder(/search experiences, categories/i).fill('temple');
    await page.waitForTimeout(1000);
    await page.getByRole('button', { name: /add/i }).first().click();
  ...
  test('should handle mobile safe area on iOS devices @p1 @mobile @sticky-bar', async ({ page }) => {
    ...
    const checkoutButton = page.locator('button').filter({ hasText: /checkout/i });
  ...
  test('should migrate guest trip data on login @p0 @sticky-bar @auth', async ({ page }) => {
    // As guest, add items to trip
    await page.goto('/explore');
    await page.waitForLoadState('networkidle');
    await page.getByPlaceholder(/search experiences, categories/i).fill('cooking');
    await page.waitForTimeout(1000);
    await page.getByRole('button', { name: /add/i }).first().click();
