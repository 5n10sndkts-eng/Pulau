...
function findStoryFiles(dir: string): string[] {
  try {
    const output = execSync(`find ${dir} -name "*.md" | grep -v "epic-"`, {
      encoding: 'utf-8',
    });
...
function parseStory(filePath: string): Story {
  ...
  const title = titleMatch
    ? titleMatch[1].replace(/^Story\s*/i, '').trim()
    : filename;

  // Extract ACs
  const acLines = content.match(/^-\s*\[[ x]\]\s*(AC\d+:?.+)$/gm) || [];
  const acceptanceCriteria = acLines.map((line) =>
...
function checkCoverage(testPaths: string[], ac: string): boolean {
  ...
  const keywords = ac
    ...
    .filter(
      (w) =>
        w.length > 4 &&
        !['should', 'ensure', 'verify', 'check'].includes(w.toLowerCase()),
    );
...
async function main() {
  ...
  for (const storyFile of files) {
    ...
    if (linkedTests.size === 0) {
      ...
      const featureKeywords = story.title
        .split(' ')
        .filter((w) => w.length > 5)
        .slice(0, 2);
  ...
  for (const res of results) {
    const icon =
      res.status === 'PASS' ? '✅' : res.status === 'FAIL' ? '⚠️' : '❌';
    const coveragePct =
      res.totalAC > 0 ? Math.round((res.coveredAC / res.totalAC) * 100) : 100;
    const testCount = res.linkedTests.length;
    const gapPreview =
      res.missingCoverage.length > 0
        ? res.missingCoverage[0].substring(0, 50) + '...'
        : 'None';
  ...
  for (const res of results.filter((r) => r.status !== 'PASS')) {
    ...
    if (res.linkedTests.length === 0)
      report += `> No linked tests found for modified files.\n`;
