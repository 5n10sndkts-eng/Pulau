...
async function validate() {
    ...
    if (!statusMap) {
        console.error('❌ CRITICAL: No development_status section found in sprint-status.yaml');
        process.exit(1);
    }
    ...
    for (const [key, status] of Object.entries(statusMap)) {
        ...
        if (key.includes('retrospective')) {
            ...
            if (retroNum) {
                ...
                const match = files.find(f => f.includes(`epic-${retroNum}-retro`) || f.includes(`epic-${retroNum}-retrospective`));
                if (match) {
                    successCount++;
                    continue;
                }
                // Also check root output?
                expectedPath = path.join(IMPLEMENTATION_DIR, `epic-${retroNum}-retrospective.md`);
            }
            ...
            const shardedPath = path.join(PLANNING_DIR, 'epics', `epic-${epicNum}.md`);
        ...
        if (!fs.existsSync(expectedPath)) {
            ...
            console.error(`❌ MISSING [${type}]: ${key} (${status}) - Expected at: ${expectedPath}`);
            missingCount++;
        } else {
            successCount++;
        }
